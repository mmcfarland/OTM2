...
plots_closest_to_point_endpoint = instance_request(
    csrf_exempt(json_api_call(login_optional(
        plots_closest_to_point))))

instance_info_endpoint = instance_request(
    csrf_exempt(json_api_call(login_optional(instance_info))))

login_endpoint = csrf_exempt(
    json_api_call(login_required(user_info)))

plots_endpoint = instance_request(
    json_api_call(
        route(POST=login_required(update_or_create_plot),
              GET=get_plot_list)))

plot_endpoint = instance_request(
    json_api_call(
        route(GET=login_optional(get_plot),
